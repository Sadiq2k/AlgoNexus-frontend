...
import { SolutionSubmitRequest } from '../../models/solution-submit-request';
...
export function runAndTestSolution(http: HttpClient, rootUrl: string, params: RunAndTestSolution$Params, context?: HttpContext): Observable<StrictHttpResponse<TestCaseRunResponse>> {
  const rb = new RequestBuilder(rootUrl, runAndTestSolution.PATH, 'post');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<TestCaseRunResponse>;
    })
  );
}

runAndTestSolution.PATH = '/test-case/run-and-test';
